...
function faveColorFinder(color){
  if (color === 'red'){
    ...
    return 'green is a solor favorite color'
  } else if (color === 'black'){
    return 'so trendy'
  } else {
    return 'you need to evaluate your favortite color choice'
  }
}
...
let newEmail = emailCheck('drstrangegmail.com')
...
function chocoFrogsBuyer(gold){
  return gold / 3 
}
let totalFrogs = chocoFrogsBuyer(20)
...
/*
  ...
  (give it the same name, just add a 2 to the end of it) that fixes this bug. Invoke the function and store the returned value to a variable 
  called `totalFrogs2`.
*/
function chocoFrogsBuyer2(gold){
  if (gold % 3 === 0){
    return gold / 3
  } else if ((gold - 1) % 3 === 0){
    return (gold - 1)/3 
...
let totalFrogs2 = chocoFrogsBuyer2(50)
...
/*
  Write a function that takes in an array of numbers as an argument. In the body of the function,
   write logic to determine if the array is in ascending order. 
